...
\*======================================================================*/
// #############################################################################
// Set time limit, error handling, and aborting
// #############################################################################
...
// #############################################################################
if (file_exists('../includes/config.php')) { // If that is there then it's installed
    // If the admincp was renamed, let's try and find it depending on the vBulletin version
    $admincpdir = '';
    require('../includes/config.php');
    if (!empty($admincpdir)) { // 3.0.x will overwrite this
        // Version 3.0.x
        chdir("../{$admincpdir}/");
    } elseif (!empty($config['Misc']['admincpdir'])) {
...
if ($usewrapper) {
    ...
    if ($config['Database']['dbtype'] && $config['MasterServer']['servername'] && $config['MasterServer']['username'] && $config['MasterServer']['password'] && $config['Database']['dbname']) {
        // Overwrite ImpExConfig.php
...
$ImpExSession = $session_state ?: new ImpExSession();
// #############################################################################
...
// #############################################################################
$ImpExDisplay = $usewrapper ? new ImpExDisplayWrapper() : new ImpExDisplay();
...
if ($ImpExSession->get_session_var('errortable') !== 'done') {
    // Just in case the session was removed and the error table is still there.
...
if ($impexconfig['sourceexists']) {
    if ($impexconfig['source']['databasetype'] === 'mssql') { // Check if mssql support is in PHP or should a connection be made via pure style .......
    ...
    $Db_source->type = (strtolower(trim($impexconfig['source']['databasetype'])) === 'mysql' || strtolower(trim($impexconfig['source']['databasetype'])) === 'mysqli') ? 'mysql' : $impexconfig['source']['databasetype'];
    ...
    if ($Db_source->link_id) { // got connected
    ...
    if (($Db_target->link_id === $Db_source->link_id) && version_compare(PHP_VERSION, '4.2.0', '<')) {
...
if ($currentmoduleworking !== NULL) {
    ...
    if ($module === '001' && $ImpExSession->get_session_var('targetsystem') === 400) {
...
// Init
// #############################################################################
if ($module !== '000' && $module !== NULL && $currentmoduleworking === false) {
    ...
    // If it's been FINISHED, then we are restarting it
