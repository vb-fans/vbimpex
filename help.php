...
require_once(IDIR . '/db_mysql.php'); // Make sure this is updated to use mysqli
...
if (is_file(IDIR . '/ImpExConfig.php')) {
    ...
    if ($impexconfig['target']['password'] !== 'password') {
        ...
        $impex_config = true;
    } elseif (file_exists(IDIR . 'includes/config.php') && !$impex_config) {
        include(IDIR . 'includes/config.php');
        $using_local_config = '<p>' . $impex_phrases['using_local_config'] . '</p>';
        $targetdatabasetype = $config['Database']['dbtype'] ?: 'mysql';
